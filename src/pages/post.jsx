    ...
    return (
        <section className="container mx-auto flex flex-col gap-8">
            <div className="flex flex-col gap-6">
                ...
                <p>Écrit par <span className="font-bold">{postAuthor.firstName} {postAuthor.lastName}</span></p>
                <p>{post.body}</p>
                ...
                {comments.length > 0 ? (
                    <div className="flex flex-col gap-3">
                        {comments.map((comment) => {
                            const isCurrentUser = user && `${user.firstName} ${user.lastName}` === comment.user.fullName;
                            return (
                                <div className="flex justify-between items-center border-b border-gray-200 pb-2" key={comment.id}>
                                    <div className="flex flex-col gap-2">
                                        <p>{comment.body}</p>
                                        <h5 className={`text-sm ${isCurrentUser ? 'font-bold' : 'text-gray-400'}`}>
